...
## 2. Universal route-planning principle

The constrained-open-space logic applies to **every route generated by BlueSky PRO**, regardless of operational purpose.

This includes, as applicable:
- reconnaissance / observation routes;
- survey, mapping and inspection routes;
- mission routes;
- transit / repositioning routes;
- return routes;
- return-to-home / alternate-return routes;
- emergency or contingency routes;
- recovery / landing approach routes;
- multi-UAV individual routes within a group mission.

The route purpose changes the mission objective and route-generation parameters; it does **not** change the fundamental spatial-feasibility rule.

### 2.1 Open-space-first rule

External and internal environmental constraints are incorporated **before route search**:

```text
NOTAM / AIRSPACE / RESTRICTIONS / TERRAIN / OBSTACLES /
ALTITUDE LIMITS / OTHER HARD CONSTRAINTS
                         ↓
...
The system therefore searches for and constructs routes **inside the currently permitted/open spatial domain**. A prohibited or unavailable corridor is not generated as a normal route candidate and is not merely rejected after route construction.

The same rule applies when the route is generated for a mission, return, reconnaissance, transit, emergency recovery or any other operational purpose.

If the environment changes after a route has been calculated, dependency tracking marks only affected route results and downstream calculations stale; the system then recalculates the affected route from the updated constrained-open-space model.

## 2.2 Canonical planning pipeline
