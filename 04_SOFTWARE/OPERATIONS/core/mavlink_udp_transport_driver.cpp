...
#endif
bool resolve_endpoint(const MavlinkUdpEndpoint& e, sockaddr_in& a) {
...
bool MavlinkUdpTransportDriver::open(const MavlinkUdpEndpoint& local) {
    ...
    if(!resolve_endpoint({host,local.port},address)){
#ifdef _WIN32
...
bool MavlinkUdpTransportDriver::sendTo(const MavlinkUdpEndpoint& remote,const std::vector<std::uint8_t>& frame){
    ...
    if(!resolve_endpoint(remote,address)){++stats_.send_failures;return false;}
#ifdef _WIN32
...
#endif
    if(source){char buffer_address[INET_ADDRSTRLEN]{};
#ifdef _WIN32
        InetNtopA(AF_INET,&peer.sin_addr,buffer_address,sizeof(buffer_address));
#else
        inet_ntop(AF_INET,&peer.sin_addr,buffer_address,sizeof(buffer_address));
#endif
        *source=MavlinkUdpEndpoint{buffer_address,ntohs(peer.sin_port)};}
