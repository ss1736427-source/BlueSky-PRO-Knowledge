...
int main() {
    OperationalSnapshot snapshot{{"prepare"}, {{"release", {"prepare"}, AutomationLevel::Automatic, true, true, true, true, 100}}};
    SelectionContext context{{true, true, true}};
    const auto selection = select_next_action(snapshot, context);
    const auto evidence = record_selection(selection, "decision-001", "orchestrator-policy-1", "state-42",
        {"release"}, {"safety-ok", "regulatory-ok", "energy-ok"}, "READY->RELEASED", "release_ack");
    ...
    OperationalSnapshot blocked{{}, {{"blocked", {"missing"}, AutomationLevel::Automatic, true, true, true, true, 100}}};
    const auto empty_selection = select_next_action(blocked, context);
    const auto empty_evidence = record_selection(empty_selection, "decision-002", "policy-1", "state-43", {"blocked"}, {"dependency"}, "", "");
