...
Capabilities AutopilotAdapterBaseline::getCapabilities() const {
    return {{"arm", "disarm", "takeoff", "land", "hold", "resume", "rtl", "gotoPosition", "startMission", "pauseMission", "resumeMission", "abortMission"},
            {"mission_upload", "mission_download", "mission_readback", "mission_compare", "flight_log_acquisition", "clean_close"}, {}};
}
...
bool AutopilotAdapterBaseline::connect() { closed_ = false; connectionState_ = ConnectionState::Connected; linkMetrics_.healthy = true; return true; }
...
CommandResult AutopilotAdapterBaseline::command(const std::string& name) {
    ...
    if (connectionState_ != ConnectionState::Connected || closed_) { result.executionState = ExecutionState::Rejected; result.error = ErrorCode::NotConnected; result.reason = "AUTOPILOT_NOT_CONNECTED"; return result; }
    result.executionState = ExecutionState::Acknowledged; result.acknowledged = true; result.reason = name; return result;
}
...
CommandResult AutopilotAdapterBaseline::close() {
    CommandResult result;
    result.commandId = identity_.vehicleId + "-CLOSE-" + std::to_string(++commandSequence_);
    result.vehicleId = identity_.vehicleId;
    if (closed_) { result.executionState = ExecutionState::Completed; result.acknowledged = true; result.reason = "ALREADY_CLOSED"; return result; }
    closed_ = true;
    connectionState_ = ConnectionState::Disconnected;
    linkMetrics_.healthy = false;
    result.executionState = ExecutionState::Completed;
    result.acknowledged = true;
    result.reason = "CLOSED_AND_ARCHIVE_READY";
    return result;
}
} // namespace bluesky::autopilot
