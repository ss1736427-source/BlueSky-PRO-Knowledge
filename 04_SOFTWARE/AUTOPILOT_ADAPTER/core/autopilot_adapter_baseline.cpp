...
AutopilotAdapterBaseline::AutopilotAdapterBaseline(Identity identity)
...
std::vector<std::string> AutopilotAdapterBaseline::getSupportedCommands() const {
    return getCapabilities().commands;
}
std::vector<std::string> AutopilotAdapterBaseline::getSupportedMissionFeatures() const {
    return getCapabilities().missionFeatures;
}
std::vector<std::string> AutopilotAdapterBaseline::getSupportedEquipmentFeatures() const {
    return getCapabilities().equipmentFeatures;
}

bool AutopilotAdapterBaseline::connect() {
    connectionState_ = ConnectionState::Connected;
    linkMetrics_.healthy = true;
    return true;
}

void AutopilotAdapterBaseline::disconnect() {
    connectionState_ = ConnectionState::Disconnected;
    linkMetrics_.healthy = false;
}
...
CommandResult AutopilotAdapterBaseline::command(const std::string& name) {
    CommandResult result;
    result.commandId = identity_.vehicleId + "-CMD-" + std::to_string(++commandSequence_);
    result.vehicleId = identity_.vehicleId;
    result.executionState = ExecutionState::Requested;
    ...
    result.executionState = ExecutionState::Acknowledged;
...
std::optional<std::string> AutopilotAdapterBaseline::compileMission(
    const bluesky::planning::Mission& mission,
    const bluesky::planning::VehicleEquipmentCapabilityProfile& capabilities) {
    (void)capabilities;
    if (mission.identity.mission_id.empty() || identity_.vehicleId.empty()) return std::nullopt;
    mission_ = mission.identity.mission_id + ":" + identity_.vehicleId;
    return mission_;
}

bool AutopilotAdapterBaseline::uploadMission(const std::string& compiledMission) {
    if (connectionState_ != ConnectionState::Connected || compiledMission.empty()) return false;
    mission_ = compiledMission;
    return true;
}

std::optional<std::string> AutopilotAdapterBaseline::downloadMission() {
    if (connectionState_ != ConnectionState::Connected || mission_.empty()) return std::nullopt;
    return mission_;
}
std::optional<std::string> AutopilotAdapterBaseline::readBackMission() { return downloadMission(); }

MissionComparison AutopilotAdapterBaseline::compareMission(
    const std::string& approvedMission, const std::string& actualMission) const {
    return {approvedMission == actualMission,
            approvedMission == actualMission ? std::vector<std::string>{} : std::vector<std::string>{"MISSION_CONTENT_MISMATCH"}};
}

std::optional<std::string> AutopilotAdapterBaseline::readBaseline() {
    if (configuration_.empty()) return std::nullopt;
    return configuration_;
}

MissionComparison AutopilotAdapterBaseline::compare(const std::string& baseline,
                                                    const std::string& actual) const {
    return compareMission(baseline, actual);
}

CommandResult AutopilotAdapterBaseline::validate(const std::string& configuration) const {
    CommandResult result;
    result.vehicleId = identity_.vehicleId;
    result.executionState = configuration.empty() ? ExecutionState::Rejected : ExecutionState::Acknowledged;
    result.acknowledged = !configuration.empty();
    result.error = configuration.empty() ? ErrorCode::InvalidState : ErrorCode::None;
    result.reason = configuration.empty() ? "CONFIGURATION_REQUIRED" : "VALID";
    return result;
}

CommandResult AutopilotAdapterBaseline::write(const std::string& configuration) {
    auto result = validate(configuration);
    if (result.acknowledged) configuration_ = configuration;
    return result;
}

std::optional<std::string> AutopilotAdapterBaseline::readBack() { return readBaseline(); }

CommandResult AutopilotAdapterBaseline::verify(const std::string& baseline) {
    auto result = validate(baseline);
    if (!result.acknowledged) return result;
    const auto actual = readBack();
    result.acknowledged = actual.has_value() && actual.value() == baseline;
