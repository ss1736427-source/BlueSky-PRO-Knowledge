...
def _write_package(root: Path) -> None:
    ...
    files = []
    for relative in (*REQUIRED_FILES[:3], "report.md", "evidence/source.log"):
        path = root / relative
        files.append({
            "path": relative,
    ...
    (root / "certification_evidence_index.json").write_text(json.dumps(index), encoding="utf-8")
...
def test_hash_mismatch_is_rejected(tmp_path: Path) -> None:
    ...
    assert "manifest.json:hash-mismatch:processed.csv" in errors
